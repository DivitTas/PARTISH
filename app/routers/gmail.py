...
async def _process_email_background(
    ...
    calendar_credentials: Credentials # Pass calendar credentials separately
):
    """
...
async def process_user_inbox(
    ...
    calendar_credentials: Credentials = Depends(get_google_credentials) # Reuse same credentials for now
):
    """
    Fetches recent emails, analyzes them for urgency and deadlines,
    and automatically creates Google Calendar events for 'Very Urgent' emails.

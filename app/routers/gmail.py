...
async def _process_email_background(
    ...
    # calendar_credentials: Credentials # Removed as calendar integration is temporarily disabled
):
    """
    ...
    try:
        ...
        if analysis.ml_urgency_score == 2 and analysis.deadline: # Very Urgent
            print(f"  -> Detected VERY URGENT email with deadline: '{analysis.deadline}'. (Calendar event creation skipped for now).")
            ...
            print(f"  -> Detected URGENT email with deadline: '{analysis.deadline}'. (Calendar event creation skipped for now).")
            start_dt, end_dt = parse_deadline_string(analysis.deadline)
            if start_dt and end_dt:
                print(f"  Parsed deadline: Start={start_dt.isoformat()}, End={end_dt.isoformat()}")
            else:
...
async def process_user_inbox(
    ...
    # calendar_credentials: Credentials = Depends(get_google_credentials) # Removed as calendar integration is temporarily disabled
):
    """
